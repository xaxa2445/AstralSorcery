...
import net.minecraft.ChatFormatting; // Sustituye a TextFormatting
import net.minecraftforge.client.event.CustomizeGuiOverlayEvent; // Sustituye a RenderGameOverlayEvent
...
public class EffectRenderEventHandler {
    ...
    private void onDebugText(CustomizeGuiOverlayEvent.DebugText event) {
        // gameSettings pasó a ser options
        // showDebugInfo pasó a ser renderDebug
        if (Minecraft.getInstance().options.renderDebug) {

            // Añadimos una línea vacía al panel izquierdo
            event.getLeft().add("");

            // Usamos ChatFormatting en lugar de TextFormatting
            String prefix = ChatFormatting.BLUE + "[AstralSorcery]" + ChatFormatting.RESET;

            event.getLeft().add(prefix + " EffectHandler:");
            event.getLeft().add(prefix + " > Complex effects: " + EffectHandler.getInstance().getEffectCount());
        }
    }
