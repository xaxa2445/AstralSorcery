...
public class WavefrontObject {
    ...
    private VertexFormat.Mode gLDrawingMode; // En 1.20.1 se usa el enum VertexFormat.Mode

    public WavefrontObject(ResourceLocation resource) throws ModelFormatException {
        this.fileName = resource.toString();
        try {
            // Cambio importante: en 1.20.1 getResource devuelve un Optional<Resource>
            Optional<Resource> res = Minecraft.getInstance().getResourceManager().getResource(resource);
            if (res.isPresent()) {
                loadObjModel(res.get().open());
            } else {
                throw new ModelFormatException("Resource not found: " + resource);
            }
        } catch (IOException e) {
    ...
    public VertexBuffer batch(BufferBuilder buf) {
        ...
        if (this.gLDrawingMode == null) return vbo;

        // RenderTypesAS debe devolver el VertexFormat correcto (ej. POSITION_COLOR_TEX_NORMAL)
        buf.begin(this.gLDrawingMode, RenderTypesAS.POSITION_COLOR_TEX_NORMAL);
        this.render(new PoseStack(), buf);
        vbo.bind();
        vbo.upload(buf.end());
        VertexBuffer.unbind();
        return vbo;
    ...
    public VertexBuffer batchOnly(BufferBuilder buf, String... groups) {
        if (this.gLDrawingMode == null) return new VertexBuffer(VertexBuffer.Usage.STATIC);

        buf.begin(this.gLDrawingMode, RenderTypesAS.POSITION_COLOR_TEX_NORMAL);

        // CORRECCIÓN: Añadimos 'new PoseStack()' como primer argumento
        this.renderOnly(new PoseStack(), buf, groups);
    ...
    public VertexBuffer batchExcept(BufferBuilder buf, String... excludedGroupNames) {
        ...
        // CORRECCIÓN: Añadimos 'new PoseStack()' como primer argumento
        this.renderExcept(new PoseStack(), buf, excludedGroupNames);
    ...
    public void render(PoseStack poseStack, VertexConsumer vb) {
        for (GroupObject groupObject : groupObjects) {
            groupObject.render(poseStack, vb); // Pasar el stack al grupo
        }
    }
    ...
    @OnlyIn(Dist.CLIENT)
// 1. Agregamos PoseStack poseStack a los argumentos
    public void renderOnly(PoseStack poseStack, VertexConsumer vb, String... groups) {
        List<String> groupList = Arrays.asList(groups);
        for (GroupObject groupObject : groupObjects) {
            if (groupList.contains(groupObject.name)) {
                // 2. Ahora le pasamos ambos argumentos al grupo
                groupObject.render(poseStack, vb);
            }
        }
    }

    @OnlyIn(Dist.CLIENT)
    public void renderExcept(PoseStack poseStack, BufferBuilder vb, String... excludedGroupNames) {
        boolean exclude;
        for (GroupObject groupObject : groupObjects) {
            exclude = false;
