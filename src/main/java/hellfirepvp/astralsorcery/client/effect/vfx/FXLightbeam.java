...
import org.joml.Matrix4f; // Importante: Usar JOML en 1.20.1
...
public class FXLightbeam extends EntityVisualFX {
    ...
    public <T extends EntityVisualFX> void render(BatchRenderContext<T> ctx, PoseStack renderStack, VertexConsumer vb, float pTicks) {
        ...
        // Renderizamos el haz de luz en 3 planos para dar volumen (0°, 120°, 240°)
    ...
    private <T extends EntityVisualFX> void renderCurrentTextureAroundAxis(VertexConsumer vb, PoseStack renderStack, BatchRenderContext<T> ctx, Vector3 renderOffset, double angle, float scale, int r, int g, int b, int a) {
        ...
        // CORRECCIÓN UV: Usamos métodos individuales sin Tuplas
        float u = ssr.getUOffset(this.getAge());
        float v = ssr.getVOffset(this.getAge());
        float uWidth = ssr.getUWidth();
        float vHeight = ssr.getVWidth();

        // En 1.20.1, last().pose() devuelve la Matrix4f de JOML
        Matrix4f matr = renderStack.last().pose();

        Vector3 vec = to.clone().add(perpTo.clone().multiply(-1)).subtract(renderOffset);
        vec.drawPos(matr, vb).color(r, g, b, a).uv(u, v + vHeight).endVertex();

        vec = to.clone().add(perpTo).subtract(renderOffset);
        vec.drawPos(matr, vb).color(r, g, b, a).uv(u + uWidth, v + vHeight).endVertex();

        vec = from.clone().add(perpFrom).subtract(renderOffset);
        vec.drawPos(matr, vb).color(r, g, b, a).uv(u + uWidth, v).endVertex();
