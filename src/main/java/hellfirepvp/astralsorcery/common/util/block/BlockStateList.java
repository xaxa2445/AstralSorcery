...
import net.minecraft.world.level.LevelReader; // IWorldReader/Level -> LevelReader
                    ...
                    BlockState.CODEC.listOf().fieldOf("blockStates").forGetter(stateList -> {
                        ...
                        stateList.configuredMatches.forEach(predicate -> {
                            predicate.validMatch.ifLeft(applicable::addAll).ifRight(block -> {
                                // CAMBIO: getStateContainer() -> getStateDefinition()
                                // CAMBIO: getValidStates() -> getPossibleStates()
                                applicable.addAll(block.getStateDefinition().getPossibleStates());
                            });
                        });
                        return applicable;
                    }))
