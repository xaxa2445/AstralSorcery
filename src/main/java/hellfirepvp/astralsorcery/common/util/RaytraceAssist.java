...
public class RaytraceAssist {
    ...
    public boolean isClear(Level world) {
        return this.forEachBlockPos(at -> {
            if (collectEntities) {
                List<Entity> entities = world.getEntitiesOfClass(Entity.class, collectBox.move(at));
            ...
            return MiscUtils.executeWithChunk(world, at, () -> {
                if (!isStartEnd(at) && !world.isEmptyBlock(at)) {
    ...
    static {
        addPassable(((world, pos, state) -> state.getFluidState().isEmpty() && state.is(net.minecraftforge.common.Tags.Blocks.GLASS)));
    }
