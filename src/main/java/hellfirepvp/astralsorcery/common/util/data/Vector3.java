...
import com.mojang.blaze3d.vertex.VertexConsumer; // Reemplaza IVertexBuilder
...
import net.minecraft.world.phys.AABB; // Reemplaza AxisAlignedBB
import net.minecraft.world.phys.Vec3; // Reemplaza Vector3d
import net.minecraftforge.api.distmarker.Dist;
import net.minecraftforge.api.distmarker.OnlyIn;
import org.joml.Matrix4f; // Nueva librería matemática en 1.20.1
...
public class Vector3 {
    ...
    public static Vector3 directionFromYawPitch(float yaw, float pitch) {
        float radYaw   = yaw   * (Mth.PI / 180F);
        float radPitch = pitch * (Mth.PI / 180F);
    ...
    public Vector3 subtract(Entity e) {
        // En 1.20.1 se usa directamente getX(), getY(), getZ()
        // o e.position().x()
        this.x -= e.getX();
        this.y -= e.getY();
        this.z -= e.getZ();
    ...
    // 2. Para Vector3i (Vectores de enteros como BlockPos)
    public Vector3 subtract(net.minecraft.core.Vec3i vec) {
        // El paquete cambió a net.minecraft.core
        this.x -= vec.getX();
        this.y -= vec.getY();
        this.z -= vec.getZ();
        return this;
    ...
    // 3. Para Vec3 (Antiguo Vector3d)
    public Vector3 subtract(net.minecraft.world.phys.Vec3 vec) {
        // Vector3d ahora es Vec3 y sus componentes son campos públicos (x, y, z)
        this.x -= vec.x;
        this.y -= vec.y;
        this.z -= vec.z;
        return this;
    ...
    // 1. Para Vec3i (Antiguo Vector3i - ahora en net.minecraft.core)
    ...
    // 2. Para Vec3 (Antiguo Vector3d - ahora en net.minecraft.world.phys)
    public double distance(net.minecraft.world.phys.Vec3 o) {
        return Math.sqrt(distanceSquared(o));
    }

    public double distanceSquared(net.minecraft.world.phys.Vec3 o) {
        // En Vec3 de la 1.20.1, x, y, z son campos (fields), no métodos
        double difX = x - o.x;
        double difY = y - o.y;
        double difZ = z - o.z;
        return difX * difX + difY * difY + difZ * difZ;
    ...
    // 1. Vector3d ya no existe, se usa Vec3
    public net.minecraft.world.phys.Vec3 toVec3() {
        return new net.minecraft.world.phys.Vec3(x, y, z);
    }

    // 2. BlockPos ahora usa métodos estáticos para creación desde doubles
    public net.minecraft.core.BlockPos toBlockPos() {
        // En 1.20.1, BlockPos.containing hace el MathHelper.floor internamente
        return net.minecraft.core.BlockPos.containing(x, y, z);
    }

    // 3. ChunkPos y MathHelper (ahora Mth)
    public net.minecraft.world.level.ChunkPos toChunkPos() {
        // MathHelper fue renombrado a Mth
        return new net.minecraft.world.level.ChunkPos(
                net.minecraft.util.Mth.floor(x) >> 4,
                net.minecraft.util.Mth.floor(z) >> 4
        );
    }
    ...
    @OnlyIn(Dist.CLIENT)
    public VertexConsumer drawPos(PoseStack.Pose matrixEntry, VertexConsumer buf) {
        buf.vertex(matrixEntry.pose(), (float) this.x, (float) this.y, (float) this.z);
        return buf;
    }
